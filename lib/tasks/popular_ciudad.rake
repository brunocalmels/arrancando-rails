...
namespace :popular_ciudad do
  ...
  task :run, %i[tipo_id id_ciudad] => [:environment] do |_task, args|
    ...
    if args[:tipo_id] == "id"
      ciudad = Ciudad.find(args[:id_ciudad].to_i)
    elsif args[:tipo_id] == "nombre"
    ...
    if ciudad && !ciudad.populada
      puts "Corriendo popular_ciudad:run para #{ciudad.nombre}"
